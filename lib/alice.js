...
(function( win, undef ) {
  ...
  hook.alice = function( cb, test ) {
    ...
    // Otherwise, if `test` is not a function, then use a dummy function
    // exposing the scroll position directly as the first argument.
    if (typeof test === "number") {
      test = positionTester(test);
    } else if (typeof test !== "function") {
      test = function( scrollPos ) { return scrollPos; };
    }
